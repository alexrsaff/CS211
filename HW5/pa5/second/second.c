...
int main(int argc, char** argv)
{
	...
	for(int runtime = 0; runtime <5; runtime++)
	{
		while(getWord(&word, fp)!=-1)
		{
			...
			if(strcmp(word,"INPUTVAR")==0 && runtime==0)
			{
			...
			else if(strcmp(word,"OUTPUTVAR")==0 && runtime==0)
			{
				if(getWord(&word, fp)==-1)
					break;
				outputnum = atoi(word);
			...
			else if(strcmp(word,"NOT")==0)
			{
				...
				int found = 0;
				for(int varnum = 0; varnum < total_vars; varnum++)
				{
			...
			else if(strcmp(word,"DECODER")==0)
			{
				...
				for(int count = 0; count<decoder_in_num;count++)
				{
					...
					{
						//printf("NOT FOUND\n");
						for(int inner_count = 0; inner_count <= count; inner_count++)
						{
							//printf("INNER COUNT: %d OUT OF: %d\n",inner_count,count);
							if(input_array[inner_count].name!=NULL)
								free(input_array[inner_count].name);
						}
						//printf("Count: %d\n", count);
						free(input_array);
						exit_me = 1;
